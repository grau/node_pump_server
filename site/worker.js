...
(() => {
  ...
  function fetchLoop() {
    return __async(this, null, function* () {
      connectWebsocket().catch((err) => {
        ...
        alert("Update loop ist abgest\xFCrtzt! Bitte die Seite neu laden!");
      });
    ...
    return __async(this, null, function* () {
      for (; ; ) {
        ...
        socket.addEventListener("message", (message) => {
          ...
          db.addData([data]).catch((err) => console.warn("Failed to store data!", { err, data, message }));
        });
        ...
        yield (0, import_delay.default)(2e3);
      }
    });
  }
  function updateFileIndex() {
    return __async(this, null, function* () {
      ...
      const indexFiles = content.split("\n").filter((line) => line.length > 0).map((line) => line.split(";")).map((line) => ({ filename: line[0], timestamp: parseInt(line[1]) }));
  ...
  fetchLoop().catch((err) => console.warn("Webworker crashed", { err }));
})();
